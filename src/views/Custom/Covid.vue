...
export default class Covid extends Vue {
  ...
  public init() {
    ...
    const confirmedColor = am4core.color("#F44336");
    const deathsColor = am4core.color("#2196F3");

    // for an easier access by key
    const colors: any = { confirmed: confirmedColor, deaths: deathsColor };

    const countryColor = am4core.color("#424242");
    const countryStrokeColor = am4core.color("#000000");
    const buttonStrokeColor = am4core.color("#ffffff");
    const countryHoverColor = am4core.color("#212121");
    const activeCountryColor = am4core.color("#000000");
    ...
    let countryDataTimeout: any;

    let currentType: any;

    let currentTypeName: any;
    ...
    const mapChart: any = container.createChild(am4maps.MapChart);
    ...
    // polygon states
    const polygonHoverState: any = polygonTemplate.states.create("hover");
    ...
    const imageTemplate: any = bubbleSeries.mapImages.template;
    ...
    imageTemplate.adapter.add("tooltipY", function(tooltipY: any, target: any) {
      return -target.children.getIndex(0).radius;
    });

    // When hovered, circles become non-opaque
    const imageHoverState: any = imageTemplate.states.create("hover");
    ...
    bubbleSeries.events.on("dataitemsvalidated", function() {
      bubbleSeries.dataItems.each((dataItem: any) => {
    ...
    imageTemplate.adapter.add("latitude", function(latitude: any, target: any) {
    ...
    const nameAndButtonsContainer: any = buttonsAndChartContainer.createChild(
      am4core.Container
    );
    ...
    const countryName: any = nameAndButtonsContainer.createChild(am4core.Label);
    ...
    const buttonsContainer: any = nameAndButtonsContainer.createChild(
      am4core.Container
    );
    ...
    const chartAndSliderContainer: any = buttonsAndChartContainer.createChild(
      am4core.Container
    );
    ...
    // Slider container
    const sliderContainer: any = chartAndSliderContainer.createChild(
      am4core.Container
    );
    ...
    const slider: any = sliderContainer.createChild(am4core.Slider);
    ...
    // play button
    const playButton: any = sliderContainer.createChild(am4core.PlayButton);
    playButton.valign = "middle";
    // play button behavior
    playButton.events.on("toggled", function(event: any) {
    ...
    valueAxis.adapter.add("max", function(max: any, target: any) {
      if (max < 5) {
        max = 5;
      }
      return max;
    ...
    valueAxis.adapter.add("min", function(min: any, target: any) {
    ...
    const seriesTypeSwitch: any = lineChart.legend.createChild(
      am4core.SwitchButton
    );
    ...
    function updateColumnsFill() {
      columnSeries.active.columns.each(function(column: any) {
    ...
    // add series
    function addColumnSeries(name: any, color: any) {
    ...
    const buttons: any = { confirmed: confirmedButton, deaths: deathsButton };
    ...
    function changeDataType(name: any) {
      currentType = name;
      ...
      bubbleSeries.dataItems.each(function(dataItem: any) {
        dataItem.setValue("value", dataItem.dataContext[currentType]);
      });

      polygonSeries.dataItems.each(function(dataItem: any) {
    ...
    function updateSeriesTooltip() {
      ...
      lineChart.series.each(function(series: any) {
        if (!series.isHidden) {
    ...
    function rollOverCountry(mapPolygon: any) {
      resetHover();
      if (mapPolygon) {
    ...
    function rollOutCountry(mapPolygon: any) {
      const image = bubbleSeries.getImageById(mapPolygon.dataItem.id);
    ...
    // rotate and zoom
    function rotateAndZoom(mapPolygon: any) {
      polygonSeries.hideTooltip();
    ...
    function getZoomLevel(mapPolygon: any) {
